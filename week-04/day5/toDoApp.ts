...
const printToDoList = (): void => {
  ...
  if (toDoContent === '') {
    ...
    for (let taskI: number = 0; taskI < toDoContentArr.length; taskI++) {
      toDoContentArr[taskI] = `${taskI + 1} - [${toDoContentArr[taskI].match(/DONE/g) !== null ? 'X' :
        ' '}] ${toDoContentArr[taskI].slice(0, -6)}`;
    }
...
const addTask = (input: string): void => {
  input.length > 2 && input.indexOf(`"`) !== -1 ?
    fs.appendFileSync(
      'myToDoList.txt', `${input.slice(input.indexOf(`"`) + 1,
        input.lastIndexOf(`"`))}*NONE*\r\n`,
      (): void => {
...
const removeNCheckTask = (input: string): void => {
  let actionN: string = input[1] === 'r' ? 'remove' : 'check';
  if (input.trim().length === 2) {
    console.log(`Unable to ${actionN}: no index provided`);
  }
  else {
    if (input[3].match(/\d/) === null) {
      console.log('Unable to ${actionN}: index is not a number');
    }
    else {
...
const runApp = (): void => {
  ...
  if (userInput !== 'lemmeout!!!') {
    userInput === '' ? printComLine() :
      userInput.slice(0, 2) === '-l' ? printToDoList() :
        userInput.slice(0, 2) === `-a` ? addTask(userInput) :
          userInput.slice(0, 2) === `-r` ? removeNCheckTask(userInput) :
            userInput.slice(0, 2) === `-c` ? removeNCheckTask(userInput) :
              argumentError();
    runApp();
  } else {
